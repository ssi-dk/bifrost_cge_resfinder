...
from bifrost_cge_resfinder import launcher as bcrl
...
def test_install_component(mydb):
    ...
    args: argparse.Namespace = bcrl.parser(["--install"])
    bcrl.run_program(args)
...
def test_pipeline(mydb):
    ...
    args = bcrl.parser(["-id", sample_id,"-out", "/bifrost_test_data/output/test1"])
    bcrl.run_program(args)
